...
def fetch_api_data(params):
    ...
    for n in range(len(r.json()["data"])):
        lines.append(
            base64.b64decode(json.loads(r.json()["data"][n]["data"])["data"])
            .decode("utf-8")
            .split(",")
        )
...
try:
    ...
    params["timestart"] = str(
        (dsold.time[-1].astype("uint64") / 1e9).astype("int").values
    )
    print(
        "starting from incremental holo file. First burst",
        dsold["time"][0].values,
        "last burst",
        dsold["time"][-1].values,
    )
...
for k in ["signalpct", "Dm"]:
    if k in dsold:
        dsold[k][dsold[k] < 0] = -9999
    dsnew[k][dsnew[k] < 0] = -9999
...
try:
    if hour == "00":
        shutil.copy(
            fildir + site + ".nc", fildir + "../wind_bak/" + site + timestr + ".nc"
        )
except:
